...
def gen_xy_test(filename, enc, lang):
...
def train_and_predict(lang):
    ...
    x_test,y_test = gen_xy_test(lang+"/dev.out", enc, lang)
    ...
    x_words = get_words(lang+"/dev.in")
    labels = get_reverse_labels(lang+"/count_y.json")
    
    counter = 0
    with open(lang+"/dev.xgboost.out", "w",encoding="utf8") as file:
